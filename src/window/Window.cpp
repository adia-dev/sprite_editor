...
namespace se
{
	...
	{
		_window =
			new sf::RenderWindow(sf::VideoMode(_width, _height), "ImGui SFML", sf::Style::Default);
		if(_window == nullptr)
		{
		...
		_imIO->IniFilename = "/Users/abdoulayedia/Projects/Dev/C++/sprite_editor/src/imgui.ini";
		...
		_currentDirectory = "/Users/abdoulayedia/Projects/Dev/C++/sprite_editor/assets";
	...
	void Window::Render()
	{
		...
		{
			lines[2 * i + 2 * viewportSize.x].color = sf::Color(50, 50, 50, 100);
			lines[2 * i + 2 * viewportSize.x + 1].color = sf::Color(50, 50, 50, 100);
			lines[2 * i + 2 * viewportSize.x].position = sf::Vector2f(0, i);
			lines[2 * i + 2 * viewportSize.x + 1].position = sf::Vector2f(viewportSize.x, i);
		}
		...
		if(ImGui::IsMousePosValid())
		{
			...
			if(shape.getGlobalBounds().contains(sf::Vector2f(mousePos.x, mousePos.y)))
			{
				ImGui::SetNextWindowPos({ImGui::GetMousePos().x, ImGui::GetMousePos().y - 45});
				ImGui::BeginTooltip();
				ImGui::Text(
					"Mouse Position: (%.1f, %.1f)", ImGui::GetMousePos().x, ImGui::GetMousePos().y);
				ImGui::EndTooltip();
			}
		}
		...
		Components::ContentBrowser(_currentDirectory);
		...
		if(ImGui::Begin("Circle manipulator"))
		{
			...
			ImGui::DragFloat2("Position", (float*)&position, 1.0f, 0, viewportSize.x);
